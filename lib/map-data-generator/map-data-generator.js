...
module.exports = (function() {
	...
	var checkCacheAndSerialize = function(dir, stops, shapes, trips, lookupTbl) {
		var cacheDir = path.join(dir, "..", "cache");
		var cacheFile = path.join(cacheDir, path.basename(dir)+'-cache.json');
		...
		var writeCache = function() {
			...
			fs.writeFile(cacheFile, JSON.stringify(obj), function(err) {
				if(err) {
					console.log(err);
				} else {
		...
		if (fs.existsSync(cacheFile)) {
			dataTime = fs.statSync(dir).mtime;
			cacheTime = fs.statSync(cacheDir).mtime;
	...
	var gen = function(gtfs, dir, cb) {
		...
		// So the parser module said that the cache is valid.
		if (fs.existsSync(cacheFile)) {
			dataTime = fs.statSync(dir).mtime;
			cacheTime = fs.statSync(cacheDir).mtime;
